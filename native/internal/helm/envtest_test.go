...
func TestListUsingKubeConfigContents(t *testing.T) {
	cleanUp, kubeConfigFile := setupEnvTest()
	defer cleanUp()
...
func TestStatusWithDebug(t *testing.T) {
	...
	if !strings.Contains(out, "USER-SUPPLIED VALUES:") {
		t.Errorf("Expected status to include user-supplied values, got %s", out)
		return
	}
	if !strings.Contains(out, "COMPUTED VALUES:") {
		t.Errorf("Expected status to include computed values, got %s", out)
		return
	}
}
